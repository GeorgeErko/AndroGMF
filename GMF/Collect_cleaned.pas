...
interface
 ...
 Uses  Classes, SysUtils {$IFDEF WIN64} Windows,{$ENDIF};
...
implementation uses LConvEncoding;
...
begin
  if Find(Item, I) then
    if not Duplicates then
      Raise EListError.Create('Duplicate item')
    else
      AtInsert(I, Item)
  else
    AtInsert(I, Item);
...
function TSortedCollection.Find(Key: Pointer; var Index: Integer): Boolean;
var
  I: Integer;
begin
  Result := Search(Key, I);
