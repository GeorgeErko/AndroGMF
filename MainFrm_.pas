...
begin
 ...
 R := TogsRect.Create;
 try
  R.XMin := PivotGeo.X - (PivotGeo.X - ZoomBaseRect.XMin) / K;
  R.XMax := PivotGeo.X + (ZoomBaseRect.XMax - PivotGeo.X) / K;
  R.YMin := PivotGeo.Y - (PivotGeo.Y - ZoomBaseRect.YMin) / K;
  R.YMax := PivotGeo.Y + (ZoomBaseRect.YMax - PivotGeo.Y) / K;
