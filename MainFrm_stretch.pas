...
uses System.SysUtils, System.Types, System.UITypes, System.Classes, System.Variants,
     ...
     FMX.Objects,
     WptForm2, newSelector, ogcDrawerCanvas, ogcBasic, objKeyMouse;
...
begin
 ...
 GmfPath := TPath.Combine(DocPath, 'Geoplan500.gmf');
 if not FileExists(GmfPath) then
 begin
  TDialogService.ShowMessage('File not found: ' + GmfPath,
...
procedure TMainForm.SetMouseObject(const Value: TKeyMouseHook);
begin
 if Value = nil then
 begin
  if FMouseObject = nil then
   FMouseObject := TKeyMouseHook.Create(Selector, nil);
 end
 else
 begin
  if FMouseObject <> Value then
  begin
   FMouseObject.Free;
   FMouseObject := Value;
  end;
...
begin
 If Drawer = nil then begin
  Drawer := TogsDrawerCanvas.Create(nil, Painter, Canvas.Scale, nil);
 ...
 MouseObject.Attach(Drawer, Selector,
   function: Single
   begin
     Result := LastCanvasScale;
   end,
   procedure
   begin
    Painter.Repaint;
...
var
 ...
 PF: TPointF;
begin
// MouseObject.PainterMouseWheel(Sender, Shift, WheelDelta, Handled);
// Exit;
...
begin
// MouseObject.PainterMouseLeave(Sender);
// Exit;
...
begin
// MouseObject.PainterDblClick(Sender);
// Exit;
...
begin
// MouseObject.PainterMouseDown(Sender, Button, Shift, X, Y);
// Exit;
...
var
 ...
 DxGeo, DyGeo: Double;
begin
// MouseObject.PainterMouseMove(Sender, Shift, X, Y);
// Exit;
...
var
 ...
 DxGeo, DyGeo: Double;
begin
// MouseObject.PainterMouseUp(Sender, Button, Shift, X, Y);
 //Exit;
...
begin
// MouseObject.PainterGesture(Sender, EventInfo, Handled);
 //Exit;
