...
interface uses objMouse, Selector, EcDot, WpTwigs, EcLot, mpMarker,
               Graphics, Collect, drawTwigs, varConsts, sysUtils, Math;
...
implementation uses MathS, EMath, WpGeo, NotLink_, WpForm14, Windows,
                    drawGrid, WptForm2, WpForm0, WptBlock,UndoColNew, Procs,
                    TwgColle, ClassMak, WpArcs, GbfwUndo, WptOle, Dialogs,
                    ustnGlobalSettings, Ver25Form, STS, UstnRec;
...
begin
 inherited;
 Marker:=TMarker.Create(Twigs.hWndParent,mtDiagCross,clRed,20,2);
 Marker.Colors[0]:=clRed;
 Marker.Colors[1]:=clBlue;
 Marker.Colors[2]:=clLime;
 Marker.Colors[3]:=clMaroon;
 addAngle:=Pi/2;
 fActiveLots:=PCollection.Create(1);
 TemporaryDot:=TDot.Create(0,0,tdNotUsed);
 markerOrthoTwig:=TTwig.Create(0);markerOrthoTwig.Coord.Insert(TDot.Create(0,0,0));
// markerOrthoTwig
 pastRotation:=0;
end;
...
begin
 Error:='0';
 try
// If not Twigs.Settings.psSaveOrthoTwigs then begin // если не надо сохранять сегменты
  OrthoTwigs.Free;OrthoTwigs:=TOrthoTwigs.Create;
// end;
 Error:='1';
 If V25<>nil then
 If TVer25Form(V25).rootKeyMouseHookClass <> nil then begin
...
begin
 ...
 Tw1:=TTwig.Create(0);Tw1.Insert(TDot.CreateAsDot(P1));Tw1.Insert(TDot.CreateAsDot(P2));
 Tw1.SetMinMax;
 Angle:=Direct_Angle(P1.XDot,P1.YDot,P2.XDot,P2.YDot);
 Tw2:=TTwig.Create(0);Tw2.Insert(TDot.Create(X-10000*Cos(Angle+Pi/2),Y-10000*sin(Angle+Pi/2),0));Tw2.Insert(TDot.Create(X+10000*Cos(Angle+Pi/2),Y+10000*sin(Angle+Pi/2),0));
 Tw2.SetMinMax;
 Twigs.InitPereParams;
   try Twigs.newPereTwigs(Tw1,Tw2,False);except _ShowMessage('objMouseSelect '+IntToStr(121));end;
...
begin
 Result:=False;
// If not Twgigs.Settings.psOrthoTwigs then exit;
 Twigs.InitPereParams;
...
//  If not((Tw is TTwigCircle) and (Tw.What = Twig_OrthoPoint)) then
  For J:=I+1 to P.Count-1 do begin
   Tw2:=P[J];
   If (Tw.Coord.Count=Tw2.Coord.Count) and (EqualAnyPoints(Tw.XMin,Tw.YMin,Tw2.XMin,Tw2.YMin)) then continue;
   try Twigs.newPereTwigs(Tw,Tw2,False);except _ShowMessage('objMouseSelect '+IntToStr(119));end;
  end;// else
 end;
 Min:=1000000;Pnt:=nil;
...
//  Writeln(I);
  if Twig.IsVisible(GPRect) then If Twig.Closed=1 then begin
...
begin
 ...
 If YGeoRasst(GNForm.Height)>GGraphSet.PaintFragment then begin
  exit;
 end;//else Writeln('yes fragment');
If useSTS and  GlobalSettings.Settings.gsLevelsSelect then With TVer25Form(V25) do begin
 emGetDotMarker(varX,varY,LastPoint,StvorLine,objPoint,usePathTwig,useGrid,False);
 If Marker.Visible then begin Marker.TwgForm:=Twigs;exit;end;
 Twigs_:=Twigs;
 try
 For I:=1 to STS.Forms.Count-1 do If STS.Forms[I]<>Twigs then begin
  If TAttrib(STS.Attr[I]).Visible=0 then continue;
  Twigs:=STS.Forms[I];
    emGetDotMarker(varX,varY,LastPoint,StvorLine,objPoint,usePathTwig,useGrid,False);
   If Marker.Visible then begin Marker.TwgForm:=Twigs; exit;end;
 end;
...
// Writeln('MVis=',Marker.Visible);
// If Marker.Visible then Writeln('=========',TForm2(Marker.TwgForm).About.ObjectName);
{  For I:=1 to STS.Count-1 do begin
  end; }
 exit;
end;
// Writeln('GetDotMarker...',TimeToStr(Time));
// Writeln((Twigs.Twigs.TwigsCount=1),' ',(Twigs.Twigs.AnyCount=0),' ',(Twigs.Twigs.Bitmaps.Count=0),' ',(mpTwig=nil));
// If (Twigs.Twigs.TwigsCount=1) and (Twigs.Twigs.AnyCount=0) and (Twigs.Twigs.Bitmaps.Count=0) and (mpTwig=nil) then Exit;
// Writeln('OrthoCount=',OrthoTwigs.CountOrthoTwigs);
// Writeln(YGeoRasst(GNForm.Height),' <> ',GGraphSet.PaintFragment);
 addAngle:=Pi/2;blockAngle:=0;
 LinkTwigs:=PCollection.Create(1);
 Marker.Remove(GCanvas);Marker.mZ:=ZNull;
...
try
 try
  ...
  {} // Marker.Size:=Marker.OriginalSize;
      Marker.AssignMarker(GlobalSettings.MarkerView.NameOf['mvGrid'],GCanvas);
      Marker.Move(GCanvas,varX,varY,MoveNone,0,'GridPath');
   ...
   // Marker.Remove(GCanvas,'DrawDot');
    If DrawDot is TPointDot then begin
     ...
     UsedOperation:=GlobalSettings.MarkerView.Checked['mvPoint'];
      Marker.AssignMarker(GlobalSettings.MarkerView.NameOf['mvPoint'],GCanvas);
    end;
    If UsedOperation then begin
     Marker.Move(GCanvas,DrawDot.XDot,DrawDot.YDot,MoveNone,0,'DrawDot');
    ...
    If GetNearestPointFromLink(LinkTwigs,XX,YY) and not(Twig is TTwigCircle) then
     If XRasst(Distance(varX,varY,XX,YY))<=Settings.psAutoDisst then begin
      ...
      Marker.AssignMarker(GlobalSettings.MarkerView.NameOf['mvInterSect'],GCanvas);
      If UsedOperation then begin
       Marker.Move(GCanvas,XX,YY,MoveNone,0,'FromLink');
       If Settings.psSaveOrthoTwigs then
        If Round(Distance(varX,varY,XX,YY)*Const_Of_DecimalCoord*1000)>0  then
         OrthoTwigs.DrawLines(GCanvas,LinkTwigs);
    ...
    If (Twig<>nil)and(not Marker.Visible) then begin
     If XRasst(Dist)<=Settings.psAutoDisst then begin
      ...
      If Twig is TTwigArc then begin
       ...
       If UsedOperation then begin
        Marker.Move(GCanvas,XX,YY,MoveNone,0,'DrawDot');
        varX:=XX;varY:=YY;
        exit;
       end;
      end else
      ...
      try Twig.ArcView:=1;
       Seg:=Twig.GetSegment(XX,YY);
        If Settings.psStvor and (StvorLine.X1<>xyNull) and (Seg<>-1) then begin
         ...
         IL.Free;
         If PD<>nil then If XRasst(Distance(PD.XDot,PD.YDot,varX,varY))<=Settings.psAutoDisst then begin
          UsedOperation:=GlobalSettings.MarkerView.Checked['mvInterSect'];
          If UsedOperation then begin
      ...
      If Seg<>-1 then begin
       D1:=Twig[Seg-1];D2:=Twig[Seg];
        // ищем пересечение перпендикуляра к линии
         If PerpendOn(D1,D2,XX1,YY1) and (XRasst(Distance(XX1,YY1,varX,varY))<=Settings.psAutoDisst) then begin
          UsedOperation:=GlobalSettings.MarkerView.Checked['mvPerpend'];
          Marker.AssignMarker(GlobalSettings.MarkerView.NameOf['mvPerpend'],GCanvas);
          Marker.Move(GCanvas,XX1,YY1,MoveNone,0,'PerpendOn');
          XX:=XX1;YY:=YY1;
//          varX:=XX;varY:=YY;
          If UsedOperation then begin
           TemporaryDot.What:=tdDop;TemporaryDot.XDot:=XX;TemporaryDot.YDot:=YY;
        ...
        // ищем пересечение с центром
         If XRasst(Distance((D1.XDot+D2.XDot)/2,(D1.YDot+D2.YDot)/2,XX,YY))<=Settings.psAutoDisst then begin
          XX:=(D1.XDot+D2.XDot)/2;YY:=(D1.YDot+D2.YDot)/2;
          UsedOperation:=GlobalSettings.MarkerView.Checked['mvCenterLine'];
          Marker.AssignMarker(GlobalSettings.MarkerView.NameOf['mvCenterLine'],GCanvas);
          If UsedOperation then begin
           TemporaryDot.What:=tdDop;TemporaryDot.XDot:=XX;TemporaryDot.YDot:=YY;
           TimerOpen;
          end;
  ...
  {}  //If Marker.Color = Marker.Colors[0] then Marker.Size:=Marker.OriginalSize else Marker.Size:=Marker.OriginalSize div 2;
       If UsedOperation then begin
        Marker.Move(GCanvas,XX,YY,MoveNone,blockAngle,'StvorLine');
        varX:=XX;varY:=YY;
        objPoint:=Twig;
        exit;
       end;
    ...
    { If (DrawTwig<>nil) then Twig:=DrawTwig else If TwigSpline<>nil then Twig:=TwigSpline;
     If Twig<>nil then If Twig.Coord.Count>2 then begin}
      D:=LastPoint;
      If XRasst(Distance(D.XDot,D.YDot,varX,varY))<=Settings.psAutoDisst then begin
       //Marker.Remove(GCanvas,'LastPoint');
       UsedOperation:=GlobalSettings.MarkerView.Checked['mvPoint'];
       Marker.AssignMarker(GlobalSettings.MarkerView.NameOf['mvPoint'],GCanvas);
       If UsedOperation then begin
        Marker.Move(GCanvas,D.XDot,D.YDot,MoveNone,0,'LastPoint');
 ...
 //    Writeln('Blocks_begin');
      ...
      If W=TWG_Point then
       If PP.isNoClosed then begin
        If PP.userObj<>nil then begin // ищем точку в блоке
         If PP.userObj.objType = TWG_Ole then begin
          //If PP.userObj.PointIn(varX,varY) then begin
          If XRasst(Distance(varX,varY,PP.XDot,PP.YDot))<=Twigs.Settings.psAutoDisst then begin
         // If PP.userObj.PointIn(varX,varY) then begin
   ...
   {}    // Marker.Size:=Marker.OriginalSize;
            Marker.AssignMarker(GlobalSettings.MarkerView.NameOf['mvPoint'],GCanvas);
            Marker.Move(GCanvas,varX,varY,MoveNone,0,'OLE');
            objPoint:=PP;
            break;
           end;
          end;
         end;
         If PP.userObj.objType = TWG_Block then begin
          try
          oldTwigs:=Twigs;
...
var D:TDot;I,N:Integer;D1,D2:Double;MinS,S:Double;
    ...
    SS:String;
begin
 MinS:=XGeoRasst(Twigs.Settings.psAutoDisst);Result:=nil;
 Aborted:=False;
 For I:=1 to Twigs.Twigs.TwigsCount-1 do begin
...
except _ShowMessage('objMouseSelect '+IntToStr(464));end;
end;
