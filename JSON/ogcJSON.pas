...
//   jsonValue: AnsiString;
  ...
  //
   function LoadDefaultObject(P: Pointer): Pointer; override;
  end;
...
begin
 ...
 If Result then
  Case jsonObj.jsonType of
   ...
   jtEndObject, jtEndArray: Parent := Parent_.jsonParent; // дл€ всех дочерних элементов
...
//                      WriteIn([Parent.ClassName]); // ошибка или исключение (дл€ отладки)
                    end;
  ...
  jtArray        :begin
                  // считываем объект, массив, значение до закрывающей скобки
                   While (S <> ']') do begin
                    ...
                    If not CreateObjectFrom(Parent, S[1]) then
                    If S = ',' then jsonObj := TjsonPropValue.Create(Parent, Prims) else
                  // дл€ корректного считывани€ значени€ инициируем TjsonPropValue.Create
  ...
  jtPropName     :begin
                  // считываем им€ свойства до двоеточи€
                   While S <> ':' do begin
                    if Self.Position = Self.Size then Goto Y;
                    Self.Read(S, 1);
                    prevSym := prevSym + S[1]; //If Length(prevSym) > 2 then Delete(prevSym, 1,1);
  ...
  jtPropValue    :begin
                   qMarkCount := 0;
                  // считываем объект, массив, значение до зак€той
                  // за исключением строкового значени€ в кавычках
                   qMark := True;
...
// инициализаци€ объекта TogsPropObject
...
initialization
// временно дл€ отладки json - парсера
